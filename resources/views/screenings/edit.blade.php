            ...
            <div class="p-6 sm:px-10 bg-white dark:bg-gray-800">
                ...
                <form action="{{ route('screening.update', $screening->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            <label for="theater_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Theater</label>
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <input type="date" name="date" id="date" value="{{ old('date', \Illuminate\Support\Carbon::parse($screening->date)->format('Y-m-d')) }}" 
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $screening->start_time) }}" 
                                class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-300">
                        </div>
                    </div>

                    <div class="mt-6">
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:bg-indigo-500 dark:hover:bg-indigo-600 dark:focus:ring-offset-gray-900">Update Screening</button>
                        <a href="{{ route('screenings') }}" class="ml-4 inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:focus:ring-offset-gray-900">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
