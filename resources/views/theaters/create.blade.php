...
@if(auth()->check() && auth()->user()->type == 'A')
    <x-app-layout>
        <div class="max-w-2xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
            <h2 class="text-2xl font-bold mb-8 text-gray-900 dark:text-gray-200">New Theater</h2>
            <form method="POST" action="/theaters" class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                @csrf
                @php
                ...
                @endphp
                <div class="mb-4">
                    <label for="inputtitle" class="{{ $labelClasses }}">Name</label>
                    <input type="text" name="name" id="inputname"
                        class="{{ $inputClasses }} border-gray-300 dark:border-gray-600">
                </div>
                
                <div class="mb-4">
                    <label for="inputphoto_filename" class="{{ $labelClasses }}">Photo Filename</label>
                    <input type="text" name="photo_filename" id="inputphoto_filename"
                        class="{{ $inputClasses }} border-gray-300 dark:border-gray-600">
                </div>
